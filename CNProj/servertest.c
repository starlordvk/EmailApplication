...
static int callback(void *NotUsed, int argc, char **argv, char **azColName)
{
  ...
  for(i = 0; i < argc-4; i++)
  {
    ...
    strcpy(images[count_pending_msg], argv[i+4]);
    count_pending_msg++;
  }
  printf("\n");
  return 0;
...
printf("Reply sent\n");
printf(" \n");
char path[50] = "/home/gk/Documents/Test/ServerImages/img";
char path1[10];
      ...
      if(pid == 0)
	     {
            ...
            strcpy(sql_query,"SELECT MESSAGE, S_TIMESTAMP, SENDER_EMAIL, SUBJECT, IMAGE FROM sender_receiver WHERE RECEIVER_EMAIL = ");
            strcat(sql_query, "'");
            strcat(sql_query, semail_id);
            ...
            for(i = 0; i < count_pending_msg; i++)
            {
              ...
              send(newsockfd, images[i], sizeof(images[i]), 0);
              sleep(2);
            }
            ...
            strcpy(sql_query, "INSERT INTO sender_receiver(SENDER_EMAIL, S_TIMESTAMP, RECEIVER_EMAIL, R_TIMESTAMP, MESSAGE, SUBJECT, IMAGE) VALUES(");
            strcat(sql_query, "'");
            strcat(sql_query, semail_id);
            strcat(sql_query, "'");
            strcat(sql_query, ", ");
