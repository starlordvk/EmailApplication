...
static int callback(void *NotUsed, int argc, char **argv, char **azColName)
{
  ...
  for(i = 0; i < argc-3; i++)
  {
    ...
    strcpy(subjects[count_pending_msg], argv[i+3]);
    count_pending_msg++;
  }
  printf("\n");
  return 0;
      ...
      if(pid == 0)
	     {
            ...
            strcpy(sql_query,"SELECT MESSAGE , S_TIMESTAMP, SENDER_EMAIL, SUBJECT FROM sender_receiver WHERE RECEIVER_EMAIL = ");
            strcat(sql_query, "'");
            strcat(sql_query, semail_id);
            ...
            for(i = 0; i < count_pending_msg; i++)
            {
              ...
              send(newsockfd, subjects[i], sizeof(subjects[i]), 0);
            }
            ...
            //Receiving the Subject of the Message
            int n1 = recv(newsockfd, subject, sizeof(subject), 0);
            subject[n1] = '\0';
            ...
            strcpy(sql_query, "INSERT INTO sender_receiver(SENDER_EMAIL, S_TIMESTAMP, RECEIVER_EMAIL, R_TIMESTAMP, MESSAGE, SUBJECT) VALUES(");
            strcat(sql_query, "'");
            strcat(sql_query, semail_id);
            strcat(sql_query, "'");
            strcat(sql_query, ", ");
